...
def generate_path2(current,start) -> list:
    """
    This function will help the agent to find the path between current and their parent cell.
    ...
    #reverse the list, start from the cell 'start'
    i = len(path)-1 #the last cell in the list is (0,0)
    path_trim = []
    flg = False
    while i >= 0:
        if path[i].x == start.x and path[i].y == start.y:
            flg = True
        if flg:
            path_trim.append(path[i])
        i = i - 1
    #path_trim: firt cell is 'start', last cell is goal
    return path_trim
...
def fun_repeated_Astar_p2(initial_maze, dim ):
    ...
    while True:
        ...
        path = generate_path2(goal_cell, start_cell)
        ...
        if isBlock is False:
            ...
            return 'solution', len(trajectory), discovered_maze, processed, explored, count_rp,count_rp1,count_rp2,total_planning_time
...
def fun_repeated_Astar_Q7_p2(initial_maze, dim ):
    ...
    while True:
        ...
        if isBlock is False:
            trajectory = generate_path(goal_cell)
            #print('end of one repeated A*')
            return 'solution', len(trajectory), discovered_maze, processed, explored, count_rp, total_planning_time
